...
class PrimeNumbersRemovalServer {
private:
    int port;
    bool running;
    ...
    std::vector<int> removePrimes(const std::vector<int>& array) {
    ...
    std::string processArray(const std::map<std::string, std::string>& params) {
        ...
        try {
            ...
            // Перевіряємо кожен елемент на простоту
            std::vector<bool> isPrimeFlags(n);
            std::vector<std::vector<int>> divisorsList(n);
            
            for (int i = 0; i < n; ++i) {
                isPrimeFlags[i] = isPrime(originalArray[i]);
                divisorsList[i] = getDivisors(originalArray[i]);
            }
            
            // Видаляємо прості числа
            std::vector<int> resultArray = removePrimes(originalArray);
            ...
            for (int i = 0; i < n; ++i) {
                if (i > 0) result << ",";
                result << (isPrimeFlags[i] ? "true" : "false");
            }
            result << "],";
            result << "\"resultArray\":[";
            ...
            for (int i = 0; i < n; ++i) {
                ...
                result << "{";
                result << "\"index\":" << i << ",";
                result << "\"value\":" << originalArray[i] << ",";
                result << "\"isPrime\":" << (isPrimeFlags[i] ? "true" : "false") << ",";
                result << "\"divisors\":[";
                ...
                result << "],";
                result << "\"removed\":" << (isPrimeFlags[i] ? "true" : "false");
                result << "}";
            }
            result << "]";
            result << "}";
...
public:
    PrimeNumbersRemovalServer(int p) : port(p), running(false) {
#ifdef _WIN32
    ...
    bool start() {
        ...
        running = true;
        std::cout << "════════════════════════════════════════" << std::endl;
        std::cout << "  Лабораторна робота 10" << std::endl;
        std::cout << "  Видалення простих чисел з масиву" << std::endl;
...
int main() {
    ...
    PrimeNumbersRemovalServer server(8080);
