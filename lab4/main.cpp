...
private:
    ...
    // Функція для реверсування цифр числа
    ...
    // Функція для отримання цифр числа як масиву
    std::string getDigitsAsString(long long number) {
        std::string digits;
        bool isNegative = number < 0;
        ...
        while (number > 0) {
            digits = std::to_string(number % 10) + digits;
            number /= 10;
        }
    ...
    std::string reverseNumber(const std::map<std::string, std::string>& params) {
        ...
        try {
            ...
            // Перевірка діапазону
            const long long MIN_VALUE = -9223372036854775807LL;
            const long long MAX_VALUE = 9223372036854775807LL;
            
            if (number < MIN_VALUE || number > MAX_VALUE) {
                result << "{\"error\":\"Число поза допустимим діапазоном\"}";
                return result.str();
            }
            ...
            result << "\"digits\":\"" << originalDigits << "\"";
            result << "},";
            result << "\"reversed\":{";
            result << "\"number\":" << reversed << ",";
            result << "\"digits\":\"" << reversedDigits << "\"";
            result << "},";
            result << "\"isNegative\":" << (number < 0 ? "true" : "false") << ",";
            result << "\"digitCount\":" << originalDigits.length() - (number < 0 ? 1 : 0);
            result << "}";
        ...
        if (path == "/api/calculate" && method == "POST") {
            ...
            if (bodyPos != std::string::npos) {
                ...
                if (params.find("number") != params.end()) {
                    std::string result = reverseNumber(params);
...
public:
    ReverseDigitsServer(int p) : port(p), running(false) {
#ifdef _WIN32
    ...
    bool start() {
        ...
        running = true;
        std::cout << "════════════════════════════════════════" << std::endl;
        std::cout << "  Лабораторна робота 4" << std::endl;
        std::cout << "  Реверсування цифр числа" << std::endl;
...
int main() {
    ...
    ReverseDigitsServer server(8080);
