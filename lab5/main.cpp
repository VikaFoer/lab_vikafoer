...
private:
    ...
    // Обчислення суми косинусів від j=2 до k+1
    double sumCos(int k) {
        double sum = 0.0;
        for (int j = 2; j <= k + 1; ++j) {
            sum += std::cos(j);
    ...
    // Обчислення суми синусів від j=1 до k
    double sumSin(int k) {
        double sum = 0.0;
        for (int j = 1; j <= k; ++j) {
            sum += std::sin(j);
    ...
    std::string calculateSum(const std::map<std::string, std::string>& params) {
        ...
        try {
            ...
            if (n < MIN_VALUE || n > MAX_VALUE) {
                result << "{\"error\":\"n має бути в діапазоні [" << MIN_VALUE << "; " << MAX_VALUE << "]\"}";
                return result.str();
            }
            ...
            for (int k = 1; k <= n; ++k) {
                ...
                if (std::abs(sinSum) < 1e-10) {
                    result << "{\"error\":\"Ділення на нуль при k=" << k << " (sinSum=" << sinSum << ")\"}";
                    return result.str();
                }
                ...
                termData["k"] = k;
                termData["cosSum"] = cosSum;
                termData["sinSum"] = sinSum;
                termData["term"] = term;
            ...
            for (size_t i = 0; i < terms.size(); ++i) {
                if (i > 0) result << ",";
                result << "{";
                result << "\"k\":" << (int)terms[i]["k"] << ",";
                result << "\"cosSum\":" << terms[i]["cosSum"] << ",";
                result << "\"sinSum\":" << terms[i]["sinSum"] << ",";
                result << "\"term\":" << terms[i]["term"] << ",";
                result << "\"partialSum\":" << terms[i]["partialSum"];
                result << "}";
            }
...
public:
    TrigonometricSumServer(int p) : port(p), running(false) {
#ifdef _WIN32
    ...
    bool start() {
        ...
        std::cout << "  Обчислення тригонометричної суми" << std::endl;
...
int main() {
    ...
    TrigonometricSumServer server(8080);
